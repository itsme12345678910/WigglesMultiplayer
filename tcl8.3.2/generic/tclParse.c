...

int
Tcl_EvalExNoCheck(interp, script, numBytes, flags)
    Tcl_Interp *interp;		/* Interpreter in which to evaluate the
...
#define NUM_STATIC_OBJS 20
	...
	/*
	 * Custom Wiggles Code hier, prüfe auf neue MP Commands!
	 *
	*/
	...
	/* Check for new command file */
	const char *filename = "remoteCommand";
	FILE *file;
	...
	if (content != NULL) {
		Tcl_EvalExNoCheck(interp, content, -1, 0);
	}
	}
	
	/* Delete Command file if possible */
	 if (file) {
